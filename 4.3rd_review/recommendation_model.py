...
def load_data():
    grades_df = pd.read_excel('/Users/maruthiwork/Desktop/ML_recommendation_system/4.3rd_review/dataset for ml recommendation system for elective.xlsx')
    courses_df = pd.read_excel('/Users/maruthiwork/Desktop/ML_recommendation_system/4.3rd_review/course details.xlsx')
...
def get_course_explanation(student_id, recommended_course, grades_df, courses_df):
    ...
    explanation += f"- Your grade in recent courses is {student_grade}\n"
    explanation += f"- Students with high grades are often recommended courses like: {list(higher_performing_courses)}"
